...
## Render — Humor Server (unified GUI + API)

The humor server serves both the API and the GUI as a single deployment. The GUI
is built at deploy time and served as static files from `gui/dist/`. No separate
GUI deployment is needed.

### Build Command

```
npm i -g pnpm@10.7.0 && pnpm install && pnpm run build:render
```
...
The GUI is a Vite/React SPA built during the Render build step and served by the
humor server at `/`. The `build:render` script builds the SDK first (required by
both server and GUI), then builds the GUI static assets into `gui/dist/`.

No separate deployment or process is needed — the humor server serves these
static files automatically with SPA fallback to `index.html`.

### Endpoints

| Path | Description |
|---|---|
| `/` | GUI (static files from `gui/dist/`, SPA fallback) |
...
| `ENABLE_ADMIN` | `false` | Set to `false` to disable `/add` and `/close` endpoints (default: enabled) |
